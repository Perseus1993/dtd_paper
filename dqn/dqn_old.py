...
env = gym.make('CartPole-v0')  # 使用gym库中的环境：CartPole，且打开封装(若想了解该环境，请自行百度)
...
class Net(nn.Module):
    def __init__(self):  # 定义Net的一系列属性
        ...
        self.action_sapce = N_ACTIONS

        self.features = nn.Sequential(
            nn.Conv2d(self.observation_space, 32, kernel_size=8, stride=4),
            nn.ReLU(),
...
for i in tqdm(range(400)):  # 400个episode循环
    print('<<<<<<<<<Episode: %s' % i)
    s = env.reset()  # 重置环境
    ...
    while True:  # 开始一个episode (每一个循环代表一步)
        ...
        s_, r, done, info = env.step_in_sim(a)  # 执行动作，获得反馈
